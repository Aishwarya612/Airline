...
class FlightListAdmin extends Component {
    ...
    */
    ...
    onView = (flight) => {
        localStorage.setItem('flight', JSON.stringify(flight));
        console.log("view : ",localStorage.getItem('flight'));
    ...
    render() {
        ...
        const flightlist = this.state.flights.map(f => {
            return (
                <div className="card m-4 " style={{width: 350,background: "#DCDCDC", height: "fit-content"}}>
                        ...
                        <br/>
                            <button className="btn btn-danger" onClick={() => this.onDelete(f.flightNumber)}>Delete</button>&nbsp;
                            <button className="btn btn-info" onClick={() => this.onEdit(f)}>Edit</button>&nbsp;
                            <button className="btn btn-success" onClick={() => this.onView(f)}>View Passengers</button>
                        </div>
        ...
        return (
            <div style={{backgroundImage: `url(${planeBG})`,overflow: 'hidden', height: '20000px'}}>
                <Header/>
                <div className="pt-5">
